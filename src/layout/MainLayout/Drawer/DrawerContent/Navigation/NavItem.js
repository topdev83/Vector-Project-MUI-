...
const NavItem = ({ item, level }) => {
  ...
  const itemIcon = item.icon ? (
    <Icon style={{ fontSize: drawerOpen ? '1rem' : '1.25rem', fill: isSelected ? '#5137BE' : '#7A7AB3' }} />
  ) : (
    false
  );
  ...
  return (
    ...
    >
        ...
        <ListItemIcon
          sx={{
            ...
            ...(!drawerOpen &&
              isSelected &&
              {
                // bgcolor: theme.palette.mode === 'dark' ? '#5137BE' : '#5137BE',
                // '&:hover': {
      ...
      {(drawerOpen || (!drawerOpen && level !== 1)) && (
        <ListItemText
          primary={
            <Typography variant="h4" sx={{ color: isSelected ? iconSelectedColor : textColor, marginLeft: '10%' }}>
              {item.title}
            </Typography>
          }
        />
